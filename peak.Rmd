...
# loading datasets. ---------------------------------------
output.path = "certain_path"
gwas = fread( file.choose() ) %>% as_tibble()
list.sign = NULL # read.table( file.choose(), header = F ) # list of significant SNPs full dataset. 
  ...
  for( var in seq_along(vars) )
  {
    ...
    if(length(stats)==0)
    {
     ...
     write.table(
      ...
      pdf(
        paste( output.path %+% "/plot/", extens, colnames(dataf2)[4+var], ".", proba, ".",type,".pdf", sep = "" ),
        width = 10, height = 8 )
...
write.table(
  out2,
  paste(output.path %+% "/Bilan.peaks.", extens, ".txt", sep = "" ),
