...
# loading libraries. --------------------------------------
#### faire en sorte que ca installe les packages si absent
library(data.table)
library(dplyr)
library(ggplot2)
library(tidyverse)
...
# loading datasets. ---------------------------------------
setwd( "/Users/pascalmilesi/upp_new/Lili_Pascal/LL_PM/Bayenv2_data/correct_data/bayenv_output/res10mins_MAF05/" )
...
head( gwas )
gwas<-na.omit(gwas)
colnames(gwas)[2]<-"xtx" # colnames for the different variables to be tested
colnames(gwas)[3]<-"Scaffold"
colnames(gwas)[6]<-"LG"

# declaring variables. ------------------------------------
vars = 2

##### Specifier colonne scaffold, ID, LG
dataf2 = 
  gwas %>% 
...
for( var in length(vars) )
{
  ...
  while( i < length(.signals) )
  {
    ...
    impl.signals = 
      c( impl.signals, dataf %>% slice(.signals[i:(j-1)]) %>% filter(avg==max(avg)) %>% pull(Pos) %>% median() %>% round())
      # c(impl.signals, mean(.signals[i:(j-1)], na.rm = T)%>% round) #dataf$Pos[.signals[which.max(dataf$avg[.signals[i:(j-1)]])]] ) #mean(.signals[i:(j-1)], na.rm = T)%>% round  
    i = j + 1
  }
...
for( var in seq_along(vars) )
{
  ...
  for( i in 1:nrow(peaklimit) )
  {
    ...
    .peak.annotation[i,7] = dataf2 %>% slice(c(.peak.annotation[i,3]:.peak.annotation[i,4])) %>%
      filter(ID %in% list.sign[,var] ) %>% pull(Scaff) %>% unique %>% length # length(unique(dataf2$ID[which(dataf2$ID[c(.peak.annotation[i,3]:.peak.annotation[i,4])]%in%list.sign[,var]==T)])
                              
    .peak.annotation[i,8] = sum(dataf2$ID[c(.peak.annotation[i,3]:.peak.annotation[i,4])]%in%list.sign[,var])# dataf2 %>% filter( ID %in% list.sign[,var] ) %>% nrow()
    snp.sign<-c(snp.sign, dataf2 %>% slice(c(.peak.annotation[i,3]:.peak.annotation[i,4])) %>%
      filter(ID %in% list.sign[,var] ) %>% pull(ID))
    
    scaff.sign<-c(scaff.sign, dataf2 %>% slice(c(.peak.annotation[i,3]:.peak.annotation[i,4])) %>%
...
if( type == "BF" || type == "Z" )
{
  for( var in seq_along(vars) )
  {
    ...
    if(length(stats)==0)
    {
      ...
      pdf(
        paste( "plot/", extens, colnames(dataf2)[4+var], ".", proba, ".",type,".pdf", sep = "" ),
        width = 10, height = 8 )
